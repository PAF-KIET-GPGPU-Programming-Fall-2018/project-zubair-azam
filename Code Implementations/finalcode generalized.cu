...
const int totalpatrn =10;
const int Ntbits=transNo*totalpatrn;
const int patternLength = 3;

	int transbit[Ntbits] =getrandoms(Ntbits);

int bits[transNo] = getbits(transNo);

	const int Nofpatrn = totalpatrn*patternLength;
	int pattern[Nofpatrn] = getitems(Nofpatrn);
